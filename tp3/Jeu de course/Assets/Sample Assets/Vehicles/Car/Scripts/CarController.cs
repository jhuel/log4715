...
public class CarController : MonoBehaviour
{
    ...
    const float MAX_SPEED_STEER_ANGLE = 0.23f;
    const float MAX_SPEED_STEER_RESPONSE = 0.5f;
    ...
    public float forceAccel = 175f;
    public int NitroReturned = 100;
    public int HpReturned = 100;
    public int nitroUsed = 10;
    //public int 
    [System.Serializable]
    ...
    private static int CHECKPOINT_VALUE = 500;
    ...
    public int collectiblePoints = 100;
    public int  DriftPoint = 1;
    public int AirPoints = 3;
    ...
    void FixedUpdate()
    {
        ...
        if (!anyOnGround && CurrentSpeed != 0)
        {
            ...
            if (airPoints == 3)
            {
                PlayerPoints += AirPoints;
                airPoints = 0;
            }
        }
        if (SpeedBonus > 0)
        {
            rigidbody.AddForce(transform.rotation * (new Vector3(0f, 0f, forceAccel)));
            SpeedBonus--;
        }
    ...
    public void Move(float steerInput, float accelBrakeInput)
    {
        ...
        if (immobilized || maxSpeed == 0) accelBrakeInput = 0;
    ...
    void OnGUI()
    {
        ...
        if (speedometer != null)
        {
            ...
            float milieu = 7/3;
            float speedmulti =  (8f - milieu) /7;

            float speedratio = CurrentSpeed / (MAX_SPEED*speedmulti);
            int angle = (int)(((1 - speedratio) * 280) - 140);
    ...
    public void shootBlue()
    {
        ...
        projectile = Resources.LoadAssetAtPath("Assets/Sample Assets/Utility/Prefabs/BlueShell.prefab", typeof(GameObject));
        clone = Instantiate(projectile, transform.position + transform.rotation * (new Vector3(0f, 0.5f, 10f)), transform.rotation) as GameObject;

        GameObject manager = GameObject.FindGameObjectWithTag("GameManager");

        clone.GetComponent<BlueShell>().isTracking = true;
        clone.GetComponent<BlueShell>().trackedObject = manager.GetComponent<RaceManager>().carOrder[0].gameObject.transform;
    ...
    public void useNitro()
    {
        ...
        {
            SpeedBonus += 15;
            carNitro -= nitroUsed; //for testing
        }
    }
